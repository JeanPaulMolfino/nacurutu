                        ...
                        <div class="descripcion">
                            ...
                            >
                                <p>
                                    ...
                                    <b-button
                                        v-b-toggle="
                                            idCategoriaToggle(categoria.id)
                                        "
                                        variant="primary"
                            ...
                            <div
                                :class="[
                                    display === 'nuevaCategoria'
                                        ? ''
                                        : 'invisible'
                                ]"
                            >
                                <b-form
                                    @submit="checkFormCategoria"
                                    @reset="onResetCategoria"
                                >
                                    <b-form-group
                                        id="input-group-1"
...
export default {
  ...
  methods: {
    ...
    async fetchSensoresPorCategoria(){
      this.loaded = false;
      this.categorias.forEach(element => {
          try {
        const response = await fetch(this.endpointSensoresPorCategoria + element.id);
        const myJson = await response.json();
        this.sensoresCategoria[element.id] = myJson;
        this.loaded = true;
      } catch (e) {
        console.error("catched! " + e);
      }
  ...
  mounted() {
    ...
    const asyncSensoresPorCategoria = async (callback, ms, triesLeft = 2) => {
      return new Promise((resolve, reject) => {
        const interval = setInterval(async () => {
          if (await this.fetchSensoresPorCategoria()) {
            resolve();
            clearInterval(interval);
          } else if (triesLeft <= 1) {
            reject();
    ...
    asyncCategorias().then(asyncSensoresPorCategoria());
    asyncInterval();
    console.log("Fetched");
  },
};
