...
class endpoints extends Controller
{
    ...
    {
        $data = json_encode(DB::select("Select s.* from dispositivos as d inner join tipos_dispositivos as td inner join sensores as s where td.id = d.id_tipo and td.id = s.id_tipo and d.identificador='" . $id . "'"), JSON_UNESCAPED_UNICODE);
        return response($data, 200)->header('Content-Type', 'application/json');
    }
